...
class DataFlow(NodeVisitor):
    ...
    def show_with_points(self, buf=sys.stdout):
        _str = ""
        # TODO It's bugged; it will only print the last function that was defined.
    ...
    def calculate_instruction_LV_gen_kill(self, instruction, point):
        # TODO Think about whether you need to consider `#define`.
        ...
        # TODO It might cause an issue when it's a store of the type `store_type_*`.
        if instruction[0].startswith("store"):
            ...
            gen = [instruction[1]]
            kill = []
        # TODO I think it's missing the consideration of `read`.
        else:
    ...
    def deadcode_elimination(self):
        ...
        for point, inst in self.code_with_program_point.items():
            if inst[0].startswith("load"):
            ...
            ):  # TODO Consider whether it's correct to disregard the `store_type_*` case.
                if inst[2] not in self.program_point_out_LV[point]:
                    self.dead_code.add(point)
                ...
                if inst[3] not in self.program_point_out_LV[point]:
                    self.dead_code.add(point)
            # TODO Maybe you could consider `elem` and `call` as well.
    ...
    def get_uses(self, inst):
        # TODO Think about whether `#define` has any use.
        # TODO Think about whether `#define` has any use.
